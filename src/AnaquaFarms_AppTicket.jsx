...
const DEFAULT_FIELDS = [
  { id: 1,  name: "North Field A",  acres: 42.5 },
  { id: 2,  name: "North Field B",  acres: 38.0 },
  { id: 3,  name: "South Field 1",  acres: 55.2 },
  { id: 4,  name: "South Field 2",  acres: 61.8 },
  { id: 5,  name: "East Pasture",   acres: 29.0 },
  { id: 6,  name: "West Pasture",   acres: 33.5 },
  { id: 7,  name: "River Bottom",   acres: 48.0 },
  { id: 8,  name: "Ridge Top",      acres: 22.3 },
  { id: 9,  name: "Front 40",       acres: 40.0 },
  { id: 10, name: "Back 80",        acres: 80.0 },
];
...
function printTicket(form, chemicals, totalAcres, fieldSchedule) {
  ...
  const thisLoadChemRows = resolvedChems.map(({ chem, effRate, partFmt, fullFmt }) => `
    <tr>
      <td>${chem.name}</td>
      <td>${chem.epa}</td>
      <td class="num">${parseFloat(effRate||0).toFixed(2)} ${chem.unit}/ac</td>
      <td class="amt partial">${partFmt || fullFmt}</td>
      <td>${chem.rei}</td>
    </tr>`).join("") || '<tr><td colspan="5" style="color:#aaa;font-style:italic;padding:8px">No chemicals entered.</td></tr>';

  // Full-tank chem rows (only used when total acres >= one full tank)
  const fullChemRows = resolvedChems.map(({ chem, effRate, fullFmt }) => `
    <tr>
      <td>${chem.name}</td>
      <td>${chem.epa}</td>
      <td class="num">${parseFloat(effRate||0).toFixed(2)} ${chem.unit}/ac</td>
      <td class="amt">${fullFmt}</td>
      <td>${chem.rei}</td>
    </tr>`).join("") || '<tr><td colspan="5" style="color:#aaa;font-style:italic;padding:8px">No chemicals entered.</td></tr>';

  // Partial load card (when full loads exist AND there are leftover acres)
  const partialTankGal  = hasPartial ? (parseFloat(partialAcres) * parseFloat(form.galPerAcre || 0)).toFixed(2) : "0";
  const partialChemRows = hasPartial ? resolvedChems.map(({ chem, effRate, partFmt }) => `
    <tr>
      <td>${chem.name}</td>
      <td>${chem.epa}</td>
      <td class="num">${parseFloat(effRate||0).toFixed(2)} ${chem.unit}/ac</td>
      <td class="amt partial">${partFmt || "—"}</td>
      <td>${chem.rei}</td>
    </tr>`).join("") : "";
  ...
  <div class="partial-card">
    <div class="partial-card-hdr">
      ⚠&nbsp; PARTIAL LOAD &mdash; ${parseFloat(partialAcres).toFixed(1)} acres &nbsp;|&nbsp; Tank Size: ${partialTankGal} gal
    </div>
    <table>
      <thead>
        <tr><th>Product Name</th><th>EPA Reg #</th><th>Rate / Acre</th><th>Add to Tank</th><th>REI</th></tr>
      </thead>
      <tbody>${partialChemRows}</tbody>
    </table>
  </div>` : "";
  ...
  <h3>Chemical Mix &mdash; This Load (${parseFloat(totalAcres).toFixed(1)} ac &nbsp;|&nbsp; ${thisLoadTankGal} gal tank)</h3>
  <table>
    <thead>
      <tr>
        <th>Product Name</th><th>EPA Reg #</th>
        <th style="text-align:right">Rate / Acre</th>
        <th style="text-align:right" class="partial">Add to Tank</th>
        <th>REI</th>
      </tr>
    </thead>
    <tbody>${thisLoadChemRows}</tbody>
  </table>` : `
  <h3>Chemical Mix &mdash; Full Tank Recipe</h3>
  <table>
    <thead>
      <tr>
        <th>Product Name</th><th>EPA Reg #</th>
        <th style="text-align:right">Rate / Acre</th>
        <th style="text-align:right">Add to Tank</th>
        <th>REI</th>
      </tr>
    </thead>
    <tbody>${fullChemRows}</tbody>
  </table>
  ${partialCard}`;
...
<style>
  * { box-sizing:border-box; margin:0; padding:0; }
  ...
  .page { padding:18px 22px; max-width:720px; margin:0 auto; }

  .header { display:flex; justify-content:space-between; align-items:flex-start;
    border-bottom:3px solid #2a5c0f; padding-bottom:8px; margin-bottom:12px; }
  .farm { font-size:18px; font-weight:800; color:#2a5c0f; letter-spacing:.02em; }
  .farm-sub { font-size:10px; color:#555; margin-top:2px; }
  .ticket-title { font-size:13px; font-weight:700; text-align:right; color:#2a5c0f; }
  .ticket-meta  { font-size:10px; color:#777; text-align:right; margin-top:2px; }

  h3 { font-size:9px; font-weight:800; color:#fff; background:#2a5c0f; padding:4px 8px;
       letter-spacing:.08em; text-transform:uppercase; margin:12px 0 0; }
  table { width:100%; border-collapse:collapse; font-size:10px; }
  th { background:#e6f5d0; color:#2a5c0f; font-size:9px; font-weight:700; padding:4px 6px;
       text-align:left; text-transform:uppercase; letter-spacing:.04em; border-bottom:1.5px solid #2a5c0f; }
  td { padding:4px 6px; border-bottom:1px solid #eee; vertical-align:middle; }
  .num { text-align:right; }
  .amt { font-weight:700; color:#2a5c0f; font-size:12px; }
  .partial { color:#c05000 !important; }

  .tank-grid { display:grid; grid-template-columns:repeat(4,1fr); gap:8px; margin:8px 0 6px;
    background:#e6f5d0; border-radius:6px; padding:10px 12px; }
  .tank-item label { display:block; font-size:9px; font-weight:700; color:#4a7a20;
    text-transform:uppercase; letter-spacing:.06em; }
  .tank-item .bigval { font-size:20px; font-weight:800; color:#2a5c0f; line-height:1.1; }
  .tank-item .sub { font-size:9px; color:#7aaa40; }
  .tank-partial-tag { color:#c05000; font-size:10px; font-weight:700; margin-top:2px; }

  .partial-card { border:2px solid #e0a020; border-radius:6px; margin-top:14px; overflow:hidden; }
  .partial-card-hdr { background:#fff4e0; color:#8a4800; font-size:10px; font-weight:800;
    padding:6px 10px; letter-spacing:.03em; border-bottom:1.5px solid #e0a020; }
  .partial-card table th { background:#fff4e0; color:#8a4800; border-bottom:1.5px solid #e0a020; }
  .partial-card table td { border-bottom:1px solid #fce8c0; }

  .notes-row { margin-top:12px; padding:6px 8px; background:#f9fdf5; border:1px solid #c8dbb0;
    border-radius:4px; font-size:10px; }
  .notes-row label { font-weight:700; color:#3a6b1a; font-size:9px; text-transform:uppercase;
    letter-spacing:.06em; display:block; margin-bottom:2px; }

  .footer { margin-top:12px; font-size:9px; color:#aaa; display:flex; justify-content:space-between;
...
export default function App() {
  ...
  const totalAcres        = form.selectedFields.reduce((s, f) => s + (parseFloat(f.acres) || 0), 0);
  ...
  const saveTicket = () => {
    ...
    const newTicket = {
      ...
      fieldSchedule,
      targetPest: form.targetPest, airTemp: form.airTemp,
    ...
    setTickets(t => editingId
      ? t.map(x => x.id === editingId ? newTicket : x)
      : [newTicket, ...t]
    );
    setForm(blank());
    setManualTank(false);
  ...
  const [newField,   setNewField]   = useState({ name:"", acres:"" });
  ...
  const handleFieldCSV = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      lines.slice(1).forEach((line, i) => {
        ...
        added.push({ id: maxId + i + 1, name: parts[0], acres: parseFloat(parts[1]) });
        imported++;
      });
  ...
  const addManualField = () => {
    ...
    setFieldLibrary(fl => [...fl, { id: nextId, name: newField.name, acres: parseFloat(newField.acres) }]);
    setNewField({ name:"", acres:"" });
  };
  ...
  const handleChemCSV = (e) => {
    ...
    reader.onload = (ev) => {
      ...
      lines.slice(1).forEach(line => {
        ...
        added.push({ id: Date.now() + Math.random(), name:p[0], epa:p[1], rei:p[2], unit:p[3]||"oz", rateMin:parseFloat(p[4])||0, rateMax:parseFloat(p[5])||0 });
        imported++;
      });
        ...
        {view === "form" && (
          <div>
            <div style={{...card, padding: isMobile ? "10px 10px" : "14px 16px"}}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:rGrid(1, 4, isMobile), gap:10, marginBottom:14 }}>
                <div>
                  ...
                  {form.equipmentType === "__other__" && (
                    <input value={form.equipmentTypeCustom||""} onChange={e => set("equipmentTypeCustom",e.target.value)}
                      style={{...inp, marginTop:5}} placeholder="Enter equipment name"/>
                  )}
                </div>
                <div>
                ...
                <div>
                  ...
                  <div style={{
                    border:"1.5px solid #a8d870", borderRadius:5, padding:"6px 10px",
                    background:"#e6f5d0", minHeight:32, display:"flex", alignItems:"center"
                  }}>
...
{!isMobile && <td style={{ padding:"3px 0", textAlign:"right", color:"#2a5c0f", fontWeight:700 }}>{fmtTime(fs.timeStart)}</td>}
              ...
              <div style={{ display:"grid", gridTemplateColumns:rGrid(1, 3, isMobile), gap:10 }}>
                <div>
                  <label style={labelStyle}>Crop / Site</label>
                  <select value={form.crop} onChange={e => set("crop",e.target.value)} style={sel}>
                ...
                <div>
                  ...
                  <input value={form.targetPest} onChange={e => set("targetPest",e.target.value)} style={inp} placeholder="e.g. Pigweed, Armyworm"/>
                </div>
                <div>
                  <label style={labelStyle}>Total Acres</label>
                  <div style={{
                    border:"1.5px solid #a8d870", borderRadius:5, padding:"6px 10px",
                    background:"#e6f5d0", display:"flex", alignItems:"center", gap:8
                  }}>
                    <span style={{ fontSize:18, fontWeight:700, color:"#2a5c0f" }}>{totalAcresDisplay}</span>
                    <span style={{ fontSize:11, color:"#6aaa30" }}>auto-sum</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div style={{...card, padding: isMobile ? "10px 10px" : "14px 16px"}}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:rGrid(1, 4, isMobile), gap:10, marginBottom:12 }}>
                ...
                <div>
                  ...
                  <input type="number" value={form.galPerAcre} onChange={e => set("galPerAcre",e.target.value)} style={inp} placeholder="0" min="0" step="0.01"/>
                </div>
              </div>
                ...
                return (
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap: isMobile ? 6 : 12, background:"#e6f5d0", borderRadius:6, padding:"12px 14px" }}>
                    <div>
                      ...
                      <div style={{ fontSize:11, color:"#7aaa40" }}>Tank ÷ Gal/Acre</div>
                    </div>
                    <div>
            ...
            <div style={{ display:"flex", gap:10, flexDirection:"column",
              ...
              <div style={{ display:"flex", gap:12 }}>
                ...
                <button onClick={() => { setForm(blank()); setManualTank(false); setEditingId(null); }} style={{
          ...
          <div>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
              <div style={{ fontWeight:700, color:"#2a5c0f", fontSize:16 }}>
                {tickets.length} Saved Ticket{tickets.length!==1?"s":""}
              </div>
<div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
                ...
                <div style={{ display:"flex", gap:6, alignItems:"center", flexWrap:"wrap" }}>
                  ...
                  <button onClick={() => {
                    ...
                    color:"#fff", border:"none", borderRadius:6, padding:"8px 14px",
            ...
            {tickets.length === 0 ? (
              ...
              <div key={t.id} style={{
                background:"#fff", border:"1.5px solid #c8dbb0", borderRadius:8,
                padding: isMobile ? "10px 10px" : "16px 18px", marginBottom: isMobile ? 8 : 14, boxShadow:"0 1px 6px rgba(0,0,0,0.06)"
              }}>
                <div style={{ display:"flex", justifyContent:"space-between", flexWrap:"wrap", gap:8, marginBottom:8 }}>
                  <div>
                    <span style={{ fontWeight:700, color:"#2a5c0f", fontSize:15 }}>{t.date}</span>
                    <span style={{ color:"#888", marginLeft:10, fontSize:13 }}>{t.crop}</span>
                  </div>
                  <div style={{ display:"flex", gap: isMobile?8:14, fontSize: isMobile?11:13, color:"#555", flexWrap:"wrap" }}>
                ...
                {/* Per-field schedule in log */}
                <div style={{ marginBottom:8 }}>
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12 }}>
                    <thead>
                      <tr>
                        {["Field","Acres","Start","End","Duration"].map(h => (
                  ...
                  <table style={{ width:"100%", borderCollapse:"collapse", fontSize:12, marginTop:8 }}>
                    <thead>
                      <tr>{["Chemical","EPA #","REI","Rate/Acre","Unit","Full Tank","Partial Load"].map(h => (
                        <th key={h} style={{ ...th, fontSize:11, padding:"5px 8px" }}>{h}</th>
                      ))}</tr>
                    </thead>
                    <tbody>
                      {t.chemicals.map((c,i) => (
                ...
                {t.notes && <div style={{ marginTop:8, fontSize:12, color:"#666" }}>Notes: {t.notes}</div>}
          ...
          <div>
            <div style={{...card, padding: isMobile ? "10px 10px" : "14px 16px"}}>
              ...
              <div style={{ fontSize:12, color:"#555", marginBottom:10, lineHeight:1.7 }}>
                CSV format: <code style={{ background:"#e6f5d0", padding:"1px 5px", borderRadius:3 }}>Field Name, Acres</code> — first row is a header and will be skipped.<br/>
                Example rows:<br/>
            ...
            <div style={{...card, padding: isMobile ? "10px 10px" : "14px 16px"}}>
              ...
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                  <thead>
                    <tr>{["Field Name","Acres",""].map(h => <th key={h} style={th}>{h}</th>)}</tr>
                  </thead>
                  <tbody>
                    {fieldLibrary.map(f => (
                      <tr key={f.id}>
          ...
          <div>
            ...
            <div style={{...card, padding: isMobile ? "10px 10px" : "14px 16px"}}>
              ...
              <div style={{ overflowX:"auto" }}>
                <table style={{ width:"100%", borderCollapse:"collapse", fontSize:13 }}>
                  <thead>
                    <tr>{["Chemical Name","EPA #","REI","Unit","Rate Range",""].map(h=>(
